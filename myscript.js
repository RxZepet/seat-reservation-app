...
function makeRows(sectionLength, rowLength, placement){
    ...
    rows.forEach( row => {
        ...
        switch(placement){
            ...
            case "right": counter = counter + (rowLength - sectionLength); break; // add 12 to counter
            default: counter = counter + ((rowLength-sectionLength)/2); break;//add 3 to counter
        }
        ...
        switch(placement){
            case "left": counter = counter + (rowLength - sectionLength); break; //add 12 to counter
            case "right": html += `<div class="label">${row}</div>`; break; // add the label
            default: counter = counter + ((rowLength-sectionLength)/2); break;//add 3 to counter
        }
    });
...
(function(){
    ...
    for(const key in reservedSeats){
        if (reservedSeats.hasOwnProperty(key)){
            ...
            document.getElementById(obj.seat).className = 'r'; //I need to figure this out to get it right
    ...
    function manageConfirmForm(){
        if(selectedSeats.length > 0){
            ...
            document.getElementById('selectedseats').innerHTML = 'You need to select some seats to reserve.<br><a href="#" id="error">Close</a> this dialog box and pick at least seat.';
